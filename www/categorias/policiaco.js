let padre = document.getElementById('policiaco');

let policiaco = JSON.parse(localStorage.getItem('Policiaco')) || [];

policiaco.forEach((element, index) => {
...
function eliminar(index) {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      ...
      policiaco = JSON.parse(localStorage.getItem('Policiaco')) || [];
      policiaco.splice(index, 1);
      localStorage.setItem('Policiaco', JSON.stringify(policiaco));
      location.reload();
    }
  });
}
function editar(index) {
  document.getElementById('newId').value = policiaco[index].id;
  document.getElementById('newNombre').value = policiaco[index].nombre;
  document.getElementById('newAutor').value = policiaco[index].autor;
  document.getElementById('newCategorias').value = policiaco[index].categoria;
  document.getElementById('newFechaP').value = policiaco[index].fechaPublicacion;
  document.getElementById('newDescripcion').value = policiaco[index].descripcion;
}
function actualizarDatos() {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      ...
      var policiaco = JSON.parse(localStorage.getItem('Policiaco')) || [];
      let elementIndex = id - 1;
      policiaco[elementIndex].nombre = nuevoNombre;
      policiaco[elementIndex].autor = nuevoAutor;
      policiaco[elementIndex].categoria = nuevaCategoria;
      policiaco[elementIndex].fechaPublicacion = nuevaFecha;
      policiaco[elementIndex].descripcion = nuevaDescripcion;
