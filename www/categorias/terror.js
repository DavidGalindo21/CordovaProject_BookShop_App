let padre = document.getElementById('terror');

let terror = JSON.parse(localStorage.getItem('Terror')) || [];

terror.forEach((element, index) => {
  ...
  <div class="card w-100" style="border:none;" >
         ...
         <p id="descripcion-${index}" class="position-absolute text-white" style="top:0;left:0;width:100%;padding:10px;background:rgba(0,0,0,.8);display:none;" >
                ${element.descripcion}
         </p>
         </div>
      <div class="card-body p-1   ">
          ...
          <h6 class="mt-3 pb-3" > ${element.fechaPublicacion} </h6>
          </div>
      </div>
...
function eliminar(index) {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      ...
      terror = JSON.parse(localStorage.getItem('Terror')) || [];
      terror.splice(index, 1);
...
function editar(index) {
  document.getElementById('newId').value = terror[index].id;
  document.getElementById('newNombre').value = terror[index].nombre;
  document.getElementById('newAutor').value = terror[index].autor;
  document.getElementById('newCategorias').value = terror[index].categoria;
  document.getElementById('newFechaP').value = terror[index].fechaPublicacion;
  document.getElementById('newDescripcion').value = terror[index].descripcion;
}
function actualizarDatos() {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      ...
      var terror = JSON.parse(localStorage.getItem('Terror')) || [];
      let elementIndex = id - 1;
      terror[elementIndex].nombre = nuevoNombre;
      terror[elementIndex].autor = nuevoAutor;
      terror[elementIndex].categoria = nuevaCategoria;
      terror[elementIndex].fechaPublicacion = nuevaFecha;
      terror[elementIndex].descripcion = nuevaDescripcion;
